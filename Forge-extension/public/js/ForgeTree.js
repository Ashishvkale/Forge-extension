$(document).ready(function () {
    ...
    switch (node.type) {
      case 'bucket':
          jQuery.ajax({
            ...
            error: function (err) {
              alert('Error while deleting bucket')
              console.log(err);
            }
          });
      break;
  ...
  function deleteObject(node) {
    $("#forgeViewer").empty();
    if (node == null) var node = $('#appBuckets').jstree(true).get_selected(true)[0];
    switch (node.type) {
      case 'object':
          jQuery.ajax({
            ...
            type:'delete',
            contentType: 'application/json',
            data: JSON.stringify({ 'bucketKey': node.parents[0] , 'objectName' : node.text}),
            success: function (res) {
              $('#appBuckets').jstree(true).refresh();
